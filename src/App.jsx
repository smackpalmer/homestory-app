...
function PropertyDetail({property,onBack,userTier='free',userRole=null,onShowPaywall,paidReports=[],regridToken}) {
  ...
  useEffect(()=>{
    ...
    fetch(`https://app.regrid.com/api/v1/search.json?query=${query}&token=${regridToken}&limit=1&return_custom=false`)
      .then(r=>r.json())
      .then(data=>{
        ...
        if(parcel) {
          setParcelData({
            ...
            assessedValue: parcel.asmtvalimpr||parcel.assessed_value||null,
            marketValue: parcel.parvaltype||parcel.market_value||null,
            lotSize: parcel.lotacres||parcel.lot_size||null,
            lastSaleDate: parcel.saledate||parcel.last_sale_date||null,
            lastSalePrice: parcel.saleprice||parcel.last_sale_price||null,
            parcelId: parcel.parcelnumb||parcel.parcel_id||null,
            zoning: parcel.zoning||null,
            absenteeOwner: parcel.mail_address_full&&!parcel.mail_address_full.toLowerCase().includes(property.address.toLowerCase()),
          });
        }
      })
      .catch(()=>{});
