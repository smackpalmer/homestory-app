...
function TerritoryScanner({properties, onSelect}) {
  ...
  const filtered = properties
    ...
    .sort((a,b)=>sortBy==="age_desc"?b.roofAge-a.roofAge:a.roofAge-b.roofAge);

  const critical = filtered.filter(p=>p.roofStatus==="critical");
  const aging = filtered.filter(p=>p.roofStatus==="aging");
  const notOurs = filtered.filter(p=>!p.ourJob);
  ...
  return (
    <div style={{paddingBottom:40}}>
      <div style={{marginBottom:16}}>
        ...
        <p style={{color:C.muted,fontSize:13,lineHeight:1.6}}>Every property in your database sorted by roof age. The ones that need replacement are your next leads.</p>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10,marginBottom:16}}>
        <div style={{background:C.red+"11",border:`1px solid ${C.red}33`,borderRadius:14,padding:16,textAlign:"center"}}>
          ...
          <div style={{color:C.dim,fontSize:10,marginTop:2}}>Roofs 18+ years old we haven't touched</div>
        </div>
        <div style={{background:C.yellow+"11",border:`1px solid ${C.yellow}33`,borderRadius:14,padding:16,textAlign:"center"}}>
          <div style={{color:C.yellow,fontSize:28,fontWeight:800}}>{aging.filter(p=>!p.ourJob).length}</div>
          <div style={{color:C.muted,fontSize:11,fontWeight:700,marginTop:4}}>Aging — Not Our Work</div>
          <div style={{color:C.dim,fontSize:10,marginTop:2}}>Roofs 10-17 years old — budget soon</div>
        </div>
      </div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:14}}>
        {[["all","All"],["notours","Not Our Work"]].map(([k,l])=>(
          <button key={k} onClick={()=>setFilter(k==="notours"?"critical":k)} style={{background:filter===k||(!properties.find(p=>p.ourJob)&&k==="notours")?"none":"none",color:C.muted,border:`1px solid ${C.border}`,borderRadius:20,padding:"5px 12px",cursor:"pointer",fontSize:11,fontWeight:700,fontFamily:"inherit",WebkitTapHighlightColor:"transparent"}}>{l}</button>
        ))}
      </div>
