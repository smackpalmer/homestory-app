  ...
  return (
    <div>
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:16,marginBottom:16}}>
        <div style={{color:C.muted,fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:10}}>📝 Notes</div>
        <textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Add observations, inspection notes, or anything relevant..." rows={3}
          style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:12,color:C.text,fontSize:14,resize:"none",boxSizing:"border-box",outline:"none",fontFamily:"inherit",lineHeight:1.7}}/>
        <div style={{display:"flex",alignItems:"center",gap:12,marginTop:10}}>
          <Btn small color={C.accent} onClick={()=>{setSaved(true);setTimeout(()=>setSaved(false),2000);}}>Save Notes</Btn>
          {saved&&<span style={{color:C.green,fontSize:12,fontWeight:700}}>✓ Saved</span>}
        </div>
      </div>
  ...
  return (
    <div style={{paddingBottom:48}}>
      ...
      <div style={{background:C.card,border:`1px solid ${C.blue}44`,borderRadius:16,padding:20,marginBottom:14}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {[["📋 Home Report — Contractor","$15"],["📋 Home Report — Homeowner","$25"],["📋 Home Report — Adjuster","$20"],["📐 Measurements (EagleView)","$18"],["📐 Measurements (Roofr)","$15"]].map(([item,price])=>(
      ...
      {/* Landlord tier */}
      <div style={{background:C.card,border:`1px solid #0ea5e944`,borderRadius:16,padding:20,marginBottom:14}}>
        <div style={{color:"#0ea5e9",fontSize:11,fontWeight:700,letterSpacing:1,textTransform:"uppercase",marginBottom:4}}>Landlord</div>
        <div style={{display:"flex",alignItems:"baseline",gap:4,marginBottom:4}}>
          <div style={{color:C.text,fontSize:22,fontWeight:800}}>$10</div>
          <div style={{color:C.muted,fontSize:14}}>/unit/year</div>
        </div>
        <div style={{color:C.muted,fontSize:12,marginBottom:16}}>For property owners and managers.</div>
        <div style={{display:"flex",flexDirection:"column",gap:7,marginBottom:20}}>
          {["Move-in & move-out inspections","Room-by-room condition records","Maintenance work orders","Tenant request portal","Rental listing page","Zillow application integration","Portfolio dashboard","Annual inspection reminders"].map(f=>(
            <div key={f} style={{display:"flex",gap:8,color:C.muted,fontSize:13}}>
              <span style={{color:"#0ea5e9",fontWeight:700,flexShrink:0}}>✓</span>{f}
            </div>
          ))}
        </div>
        <Btn full color="#0ea5e9" onClick={onUpgrade}>Get Landlord Access</Btn>
        <div style={{color:C.dim,fontSize:11,textAlign:"center",marginTop:6}}>$10/unit/yr · 6 units = $60/yr · 100 units = $1,000/yr</div>
      </div>
...
// ── Fast Field Log ────────────────────────────────────────────────────────────
function FastFieldLog({properties, setProperties, onDone}) {
  ...
  const handleSave = async () => {
    ...
    const event = {
      ...
      year: parseInt(year),
      type: selectedType,
      label: `${tc.label} — Completed`,
      note: (note || `${tc.label} work completed.`) + (jobNum ? " Job #"+jobNum : ""),
      source: "Our Work",
      verified: true,
      ourJob: true,
    ...
    if(matchedProp) {
      ...
      if(selectedType === "roof" && selectedTags.includes("full_replacement")) { updated.lastRoof=parseInt(year); updated.roofAge=new Date().getFullYear()-parseInt(year); updated.roofStatus=roofStatus(updated.roofAge); }
      setProperties(prev => prev.map(p => p.id === matchedProp.id ? updated : p));
    } else {
      const age = selectedType==="roof" ? new Date().getFullYear()-parseInt(year) : 20;
      setProperties(prev => [{id:`p-${Date.now()}`,address,city,state:"IL",zip:"",lat:null,lng:null,yearBuilt:1985,sqft:1600,stories:1,style:"Ranch",lastRoof:selectedType==="roof"?parseInt(year):2005,roofMaterial:"Unknown",roofWarranty:"Unknown",roofAge:age,roofStatus:roofStatus(age),ourJob:true,notes:"",claimHistory:[],timeline:[event],ownerName:""}, ...prev]);
    }
      setSaved(true);
      setStep(4);
    } catch(e) {
  ...
  if(step===4) return (
    <div style={{textAlign:"center",padding:"40px 0"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        <Btn full color={C.accent} onClick={()=>{setStep(1);setAddress("");setSelectedType(null);setNote("");setJobNum("");setSaved(false);setSelectedTags([]);setCocFile(null);}}>Log Another Job</Btn>
      ...
      {step===3&&tc&&(
        <div>
          ...
          <div style={{background:C.card,border:`1px solid ${C.accent}33`,borderRadius:12,padding:14,marginBottom:16}}>
            <div style={{color:C.accent,fontSize:11,fontWeight:700,letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>📄 Certificate of Completion (optional)</div>
            <div style={{color:C.muted,fontSize:12,lineHeight:1.6,marginBottom:10}}>Upload your COC and AI will extract the date, materials, and warranty. Attaches permanently as verified proof of work.</div>
            {!cocFile ? (
              <div onClick={()=>cocRef.current.click()} style={{border:`2px dashed ${C.border}`,borderRadius:10,padding:"16px",textAlign:"center",cursor:"pointer",WebkitTapHighlightColor:"transparent"}}>
                <div style={{fontSize:28,marginBottom:4}}>📎</div>
                <div style={{color:C.muted,fontSize:12}}>Tap to upload COC PDF</div>
              </div>
            ) : (
              <div style={{display:"flex",alignItems:"center",gap:10,background:C.green+"11",border:`1px solid ${C.green}33`,borderRadius:10,padding:"10px 12px"}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{color:C.dim,fontSize:11}}>Will be attached as verified proof</div>
                </div>
...
function PropertyDetail({property,onBack,userTier='free',userRole=null,onShowPaywall,paidReports=[]}) {
  ...
  return (
    <div style={{paddingBottom:60}}>
      ...
      <div style={{background:`linear-gradient(135deg,${C.card},${C.card2})`,border:`1px solid ${C.border}`,borderRadius:18,padding:22,marginBottom:16}}>
        ...
        <div style={{color:C.text,fontSize:21,fontWeight:800,letterSpacing:-0.5,marginBottom:2}}>{property.address}</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
          {[["Built",property.yearBuilt],["Last Roof",property.lastRoof],["Records",property.timeline.length]].map(([k,v])=>(
      ...
      {tab==="contractor"&&(
        <div>
          ...
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:16,marginBottom:14}}>
            ...
            {[["Address",`${property.address}, ${property.city}, ${property.state}`],["Owner",property.ownerName||"—"],["Year Built",property.yearBuilt],["Style",`${property.style} — ${property.stories} story`],["Sq Footage",`${property.sqft?.toLocaleString()} sq ft`],["Last Roof",property.lastRoof],["Material",property.roofMaterial],["Warranty",property.roofWarranty]].map(([k,v])=>(
  ...
  return (
    <div style={{paddingBottom:60}}>
      ...
      <div style={{marginBottom:40}}>
        ...
        <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:28,lineHeight:1.1,letterSpacing:-0.5,marginBottom:24}}>One database. Four audiences.</h2>
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {[
            ...
            {icon:"🏘️",title:"Landlords",sub:"Protect your investment with documented inspections and fast maintenance.",color:"#0ea5e9",
              items:["Move-in & move-out inspections","Tenant maintenance request portal","Work orders to maintenance team","Rental listings with Zillow apply link"]},
...
// ── Search Screen ─────────────────────────────────────────────────────────────
function SearchScreen({properties,onSelect,regridToken,setRegridToken}) {
...
function ContractorDashboard({properties, onSelect, onLogJob, userTier='free', onUpgrade, regridToken, setRegridToken}) {
  ...
  return (
    <div style={{paddingBottom:48}}>
      ...
      {/* Sub nav */}
      <div style={{display:"flex",gap:6,marginBottom:18,overflowX:"auto",paddingBottom:2,WebkitOverflowScrolling:"touch"}}>
        {[["dashboard","Dashboard"],["verified","🏅 Verified"]].map(([k,l])=>(
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:16,marginBottom:20}}>
        <div style={{color:C.muted,fontSize:11,fontWeight:700,letterSpacing:1,textTransform:"uppercase",marginBottom:12}}>Integrations</div>
        {[
          {name:"CompanyCam",icon:"📱",status:"Connect to sync photos automatically",connected:false},
          {name:"Roofr",icon:"🏗️",status:"Connect to sync jobs automatically",connected:false},
          {name:"Zapier",icon:"⚡",status:"Connect any CRM via webhook",connected:false},
        ].map(int=>(
          <div key={int.name} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:`1px solid ${C.border}22`}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{color:C.dim,fontSize:11,marginTop:1}}>{int.status}</div>
            </div>
            <div style={{background:int.connected?C.green+"22":C.surface,border:`1px solid ${int.connected?C.green+"44":C.border}`,borderRadius:20,padding:"4px 12px",color:int.connected?C.green:C.muted,fontSize:10,fontWeight:700,flexShrink:0}}>
        ...
        <div style={{padding:"12px 0",borderBottom:`1px solid ${C.border}22`}}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:8}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{color:C.dim,fontSize:11,marginTop:1}}>Year built, sq ft, owner, assessed value — auto-fills property records</div>
            </div>
          ...
          <div style={{color:C.dim,fontSize:10,marginTop:4}}>Get your token at regrid.com/api · $99/mo for full access</div>
        </div>
        <div style={{color:C.dim,fontSize:11,marginTop:10}}>Requires server — start at your laptop with <code style={{background:C.surface,padding:"1px 5px",borderRadius:3,fontSize:10}}>node server.js</code></div>
      </div>
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'DM Sans','Helvetica Neue',sans-serif",color:C.text,maxWidth:1100,margin:"0 auto"}}>
      ...
      <div style={{background:"rgba(8,11,18,0.92)",backdropFilter:"blur(20px)",borderBottom:`1px solid ${C.border}`,position:"sticky",top:0,zIndex:100}}>
        ...
        {(screen==="search"||screen==="log"||screen==="contractor"||screen==="pricing"||screen==="standards"||screen==="cloud"||screen==="sale_inspection"||screen==="csv_import")&&(
